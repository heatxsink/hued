...
var (
	...
	username_api_key string = ""
	hostname string = ""
	hue_hostname string = ""
	local_web_server_flag bool = true
	web_bind_address string = "127.0.0.1"
	web_bind_port int = 9000
)
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: hued-web -config=[string]\n")
	flag.PrintDefaults()
	os.Exit(2)
}

func init() {
	flag.StringVar(&config_filename, "config", "", "The configuration file that has your philips hue username api key in it.")
	flag.Parse()
	flag.Usage = usage
}
